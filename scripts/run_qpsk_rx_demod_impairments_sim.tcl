#!/usr/bin/env tclsh
# Run behavioral simulation for tb_qpsk_rx_demod_impairments in batch mode.
# Usage:
#   vivado -mode batch -source scripts/run_qpsk_rx_demod_impairments_sim.tcl

set script_dir [file normalize [file dirname [info script]]]
set repo_root  [file normalize [file join $script_dir ".."]]
set xpr_path   [file join $repo_root "Ez_QPSK.xpr"]
set sim_top    "tb_qpsk_rx_demod_impairments"
...
add_file_if_missing sim_1     [file join $repo_root "Sim" "tb_qpsk_rx_demod_impairments.v"]
...
if {[string first {[TB_QPSK_RX_IMPAIR][FAIL]} $sim_text] >= 0} {
    puts "ERROR: simulation reported \[TB_QPSK_RX_IMPAIR\]\[FAIL\]. See $sim_log"
    exit 1
}

if {[string first {[TB_QPSK_RX_IMPAIR][PASS]} $sim_text] < 0} {
    puts "ERROR: simulation did not report \[TB_QPSK_RX_IMPAIR\]\[PASS\]. See $sim_log"
    exit 1
}

puts "INFO: RX demod impairments simulation completed."
