#!/usr/bin/env tclsh
# Run behavioral simulation for tb_qpsk_rx_demod_random_external with the
# signal appearing late: the ADC stays at idle DC plus noise until well after
# the first blind coarse sweep has completed, so lock must come from the
# explicit re-acquisition watchdog restarting the sweep.
# Usage:
#   vivado -mode batch -source scripts/run_qpsk_rx_demod_late_start_sim.tcl
...
set_property verilog_define {QPSK_RX_RANDOM_LATE_START=1} [get_filesets sim_1]
...
puts "INFO: RX demod late-start re-acquisition simulation completed."
