...
本目录存放 Ez_QPSK 的 Vivado/XSim testbench，用于验证阶段 1 的发送、接收搬运和顶层数字回环。
...
该脚本会打开工程根目录下的 `Ez_QPSK.xpr`，设置 `tb_qpsk_tx_single_dac_min` 为仿真顶层并运行到 testbench 结束。
...
2. `tb_rx_chain_min.v`：确认 RX 链路数据连续性和固定包长 `tlast` 节奏。
3. `tb_pl_comm_top_fixed_cfg_loopback.v`：确认顶层固定配置下 AXIS 数据、`tkeep`、`tlast` 和回压保持行为正确。
