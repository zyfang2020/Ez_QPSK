...
- `tb_qpsk_rx_demod_random_external.v`：直接生成独立符号率、带 `+15 kHz/-15 kHz`、宽压测 `+35 kHz/-35 kHz` 和锁后 `+15 kHz -> +35 kHz` 漂移的残余载波偏差、慢幅度/DC 漂移和 ADC 采样噪声的 PRBS QPSK 输入，验证无 Gray 循环先验时的锁前频偏扫描、盲锁、锁后 NCO 频偏微调和符号恢复。
...
5. `tb_qpsk_rx_demod_random_external.v`：确认 RX demod 不依赖本地 Gray 循环也能盲锁并恢复带 `+15 kHz/-15 kHz`、宽压测 `+35 kHz/-35 kHz` 和锁后 `+15 kHz -> +35 kHz` 漂移的残余载波偏差、慢幅度/DC 漂移和 ADC 采样噪声的 PRBS QPSK 符号。
...
- 2026-06-11：新增 `run_qpsk_rx_demod_random_external_drift_sim.tcl`，在随机 PRBS 外部输入已锁后把残余载波从 `+15 kHz` 漂到 `+35 kHz`；仿真通过，`locked_symbols=3200`，`valid_symbols=12589`，最终 `nco_corr=5715`。
